...
$id = $_GET['id'];
$qry ="SELECT * FROM classroom WHERE id=$id";
...
?>
     ...
     <form action="actionclassroom.php" method="POST">
        ...
        <input type="file" name="grade" value="<?php echo $row['image']; ?>" 
  class="p-2 bg-gray-100 border rounded w-full block my-3">
        <input type="text" name="name" value="<?php echo $row['description']; ?>" placeholder="Description"
  class="p-2 bg-gray-100 border rounded w-full block my-3">

        <div class="my-5 flex justify-center gap-5">
            <input type="submit" name="update" value="Update class" class="bg-blue-500 text-white px-4 
